...
export default class SyncCollectorOfTasks<T, R> {
  ...
  private resolveActionPromise: (response: R) => void;
  private rejectActionPromise: (error: Error) => void;

  constructor(
    private readonly action: (params: T[]) => R,
  ...
  private pushCache({queryParams, result}) {
