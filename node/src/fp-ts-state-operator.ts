 ...
 **/
const f = state.of(1); // the `of` function constructs a state monad whose computation result fixed by argument passed to
assert.deepStrictEqual(f([]), [1, []]);

const t1 = state.of<number, number>(1);
const f7 = state.map((n: number) => n + 1)(t1); // the `map` operation modifies the computation result and left state not touched
